...
BOOL CMFCApplication1Dlg::OnInitDialog()
{
	...
	if (0 == fopen_s(&p_file, "lena_gray.raw", "rb")) {
		// 이미지 파일을 모두 읽는다.
		fread(p_src_pos, m_Re_height * m_Re_width, 1, p_file);
		// 8비트 이미지를 32비트 이미지로 확장한다. 하나의 색상을 R, G, B에 동일하게 
		// 복사하면 회색으로 처리된다. 그리고 이 방식에서는 Alpah 값이 적용되지는 않지만 
		// 불투명도 100%를 의미하는 0xFF값으로 대입한다.
		CMFCApplication1Dlg::OnBinarization();
		CMFCApplication1Dlg::OnBinaryOpening();
...
void CMFCApplication1Dlg::OnBinaryErosion() {
	...
	for (i = 0; i < m_Re_height; i++) {
		for (j = 0;j<m_Re_width; j++) {
			temp[(i + 1) * 514 + (j + 1)] = p_src_image[i*m_Re_height+j];
		}
	}
	for (i = 0; i < m_Re_height; i++) {
		for (j = 0; j < m_Re_width; j++) {
			...
			if (S == 9.0)
				p_src_image[i * m_Re_width + j] = (unsigned char)255.0;
			// 값이 모두 일치하면 출력 값은 255
			else
				p_src_image[i * m_Re_width + j] = (unsigned char)0.0;
			// 모두 일치하지 않으면 출력 값은 0
			S = 0.0; // reset
		}
	}
