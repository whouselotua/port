...
BOOL CMFCApplication1Dlg::OnInitDialog()
{
	...
	if (0 == fopen_s(&p_file, "512_512.raw", "rb")) {
		...
		CMFCApplication1Dlg::OnFrameSub();
...
void CMFCApplication1Dlg::OnFrameSub() {
	FILE* t_file;
	if (0 == fopen_s(&t_file, "lena_gray.raw", "rb"))
		fread(m_OutputImage, m_Re_height * m_Re_width, 1, t_file);
	int i;
	for (i = 0; i < m_Re_size; i++) {
		if (p_src_image[i] - m_OutputImage[i]<0)
			p_src_image[i] = 0;
		else
			p_src_image[i] = p_src_image[i] - m_OutputImage[i];
	}
