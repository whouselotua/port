...
BOOL CMFCApplication1Dlg::OnInitDialog()
{
	...
	if (0 == fopen_s(&p_file, "lena_gray.raw", "rb")) {
		// 이미지 파일을 모두 읽는다.
		...
		CMFCApplication1Dlg::OnFrameComb();
...
void CMFCApplication1Dlg::OnFrameComb() {
	FILE* t_file, * m_file;
	int i;
	unsigned char* temp = new unsigned char[512 * 512];
	unsigned char* mask = new unsigned char[512 * 512];
	unsigned char value;
	if (0 == fopen_s(&t_file, "512_512.raw", "rb")) {
		fread(temp, m_Re_height * m_Re_width, 1, t_file);
	}
	if (0 == fopen_s(&m_file, "LENNAMask512.raw", "rb")) {
		fread(mask, m_Re_height * m_Re_width, 1, m_file);
	}
	for (i = 0; i < m_Re_size; i++) {
		value = 255 - mask[i];
		p_src_image[i] = (m_OutputImage[i] & mask[i]) | (temp[i] & value);
	}
}