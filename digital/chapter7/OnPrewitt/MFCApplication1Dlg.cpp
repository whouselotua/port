...
{
	int i, j;
	double PrewittMask_R[3][3] = { {-1., -1., -1.}, {0., 0., 0.}, {1., 1., 1.} };
	double PrewittMask_C[3][3] = { {1.,0.,-1.},{1.,0.,-1.},{1.,0.,-1.} };
	for (i = 0; i < m_Re_size; i++) {
		m_OutputImage[i] = p_src_image[i];
	}
	OnMaskProcess(p_src_image, PrewittMask_R);
	OnMaskProcess(m_OutputImage, PrewittMask_C);
	for (i = 0; i < m_Re_size; i++) {
		p_src_image[i] = abs(p_src_image[i]) + abs(m_OutputImage[i]);
	}
	for (i = 0; i < m_Re_height; i++) {
		for (j = 0; j < m_Re_width; j++) {
